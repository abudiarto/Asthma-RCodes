...
### Feature 3, BMI calculated from height and weight ###
feature_folder='BMI'
# file 1
kfile=1
...
### Feature 3, BMI new directly from read code###
feature_folder='BMI'
# file 1
kfile=1
...
for (kfile in c(2:num_files)){
  ...
  feature_BMI<-feature_BMI %>% 
    rename('BMINew'= 'BMI')
  data_features$BMINew[match(feature_BMI$patid,data_features$patid)]=feature_BMI$BMINew
...
### Feature 18b, Prior asthma management ###
...
### Feature 18c, prior asthma annual review ###
...
### Feature 18d, prior asthma medication review ###
feature_folder='PriorAsthmaMedReview'
# file 1
kfile=1
load(paste(data_load_folder,'/',feature_folder,'/feature_priorAsthmaMedReview',kfile,'.Rdata',sep=''))

data_features<-data_features%>%
    left_join(feature_priorAsthmaMedReview,by='patid')
# remaining files
for (kfile in c(2:num_files)){
  print(paste('Processing file number:',kfile))
  load(paste(data_load_folder,'/',feature_folder,'/feature_priorAsthmaMedReview',kfile,'.Rdata',sep=''))

  data_features$numAsthmaMedReview[match(feature_priorAsthmaMedReview$patid,data_features$patid)]=feature_priorAsthmaMedReview$numAsthmaMedReview
}
rm(list=setdiff(ls(),c("data_features","data_load_folder","num_files")))

### Feature 18e, prior asthma review RCP###
feature_folder='PriorAsthmaReviewRCP'
# file 1
kfile=1
load(paste(data_load_folder,'/',feature_folder,'/feature_priorAsthmaReviewRCP',kfile,'.Rdata',sep=''))

data_features<-data_features%>%
    left_join(feature_priorAsthmaReviewRCP,by='patid')
# remaining files
for (kfile in c(2:num_files)){
  print(paste('Processing file number:',kfile))
  load(paste(data_load_folder,'/',feature_folder,'/feature_priorAsthmaReviewRCP',kfile,'.Rdata',sep=''))

  data_features$numAsthmaReviewRCP[match(feature_priorAsthmaReviewRCP$patid,data_features$patid)]=feature_priorAsthmaReviewRCP$numAsthmaReviewRCP
}
rm(list=setdiff(ls(),c("data_features","data_load_folder","num_files")))
...
save(data_features,file='../FinalData/data_features_2vs1_2016-2018_21102024.Rdata')
write.csv(data_features,file='../FinalData/data_features_2vs1_2016-2018_21102024.csv')
...
library(tidyverse)
load('../FinalData/selectedPatients_13Oct2020.Rdata')
data_load_folder='../PrepareDataNew'
num_files=74
data_outcomes=selectedPatients
...
save(data_outcomes,file='../FinalData/data_outcomes_2vs1_15112023.Rdata')
write.csv(data_outcomes,file='../FinalData/data_outcomes_2vs1_15112023.csv')
```
...
Outcomes (updated where we have an outcome for every 3 months) - experiment without medication
```{r}
rm(list=ls())
library(tidyverse)
load('../FinalData/selectedPatients_13Oct2020.Rdata')
...
save(data_outcomes,file='../FinalData/data_outcomes_experiment_17102024.Rdata')
write.csv(data_outcomes,file='../FinalData/data_outcomes_experiment_17102024.csv')
```
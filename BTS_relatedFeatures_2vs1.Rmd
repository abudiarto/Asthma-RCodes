  ...
  f_therapy_part <- f_therapy_part %>%
    mutate(
      BTS_step = case_when(
        ...
        (flag_ICS_LABA |
           flag_ICS) & ICS_dose == 'medium' & !(flag_OCS) ~ "3",
        (flag_ICS_LABA |
           flag_ICS) & ICS_dose == 'high' & !(flag_OCS) ~ "4",
        flag_OCS ~ "5",
        TRUE ~ "unknown"
      )
    )
   ...
   save(feature_BTSstep,file=paste('../PrepareDataNew_20162018/BTS_Step/feature_BTSstep_',kc,'.Rdata',sep=''))
   save(feature_ICSinfo,file=paste('../PrepareDataNew_20162018/ICS_info/feature_ICSinfo',kc,'.Rdata',sep=''))
